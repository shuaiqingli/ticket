...
function getcity(cityName){
	...
	ajaxAsync(params,"public/api/getAllCity", function(json){
		citys = json.datas;
		jeach(json.datas, function(k,v){
			varAddressList+='|1@'+v.stpinyin+'|'+v.cityname+"|"+JSON.stringify(v)+"|"
		});
		varAddressList+='1@';
		citylist = json.datas;
	})
...
function c$(fctId,fctClassName){
	...
	if(!varTempDivObj){
		...
		document.getElementById('search_box').appendChild(varTempDivObj);
	}
...
function GetIframe(fctIfmId,fctVisibility,fctTop,fctLeft,fctWidth,fctHeight){
	...
	if(!frmCity){
		...
		document.getElementById('search_box').appendChild(frmCity);
	}
...
//    varAddressList = '|1@xuanhuaqu|宣化|1@hengdianzhen|横店|1@';
...
//		var varCityDataSplitI = varCityDataSplit[i];
//		varCityDataSplitISplit=varCityDataSplitI.split("|");
//		console.debug(varCityDataSplit[i]);
//		console.debug(varCityDataSplitI +'******' + varObjValue)
//		if(varCityDataSplitI.toUpperCase().indexOf(varObjValue.toUpperCase())!=-1|| varCityDataSplitISplit[2].toUpperCase().indexOf(varObjValue.toUpperCase())!=-1|| varCityDataSplitISplit[1].toUpperCase().indexOf(varObjValue.toUpperCase())!=-1 || varObjValue=="" || i==varObjValue){ // || varCityDataSplitI.toLowerCase().indexOf(varObjValue.toLowerCase())>=0
//			varThisPageI+=1;
//			if(varThisPageI>PageId*varPageRCount && varThisPageI<=(PageId+1)*varPageRCount){
//				//varCityDataSplitISplit=varCityDataSplitI.split("|");
//				varHtmlStr+="<a href='javascript:;' onclick='GetCity("+varThisPageI+")' id='menuA"+varThisPageI+"' title='"+varCityDataSplitI+"'><span style='display:none;'>"+varCityDataSplitISplit[0]+"&nbsp;&nbsp;</span>"+varCityDataSplitISplit[1]+"</a>";//("+varCityDataSplitISplit[2]+")
//			}			
//		}
//	}
...
//		console.debug(v)
		varHtmlStr+="<a href='javascript:;' city='"+JSON.stringify(v)+"' onclick='GetCity(this)' id='menuA"+i+"' title='"+v.cityname+"'><span style='display:none;'>"+v.cityname+"&nbsp;&nbsp;</span>"+v.cityname+"</a>";//("+varCityDataSplitISplit[2]+")
//		$('#divAddressMenu').append(varHtmlStr);
	})
...
//		varNextPageStr+="<span id=menuPageE style="+(varThisPageI>(PageId+1)*varPageRCount?"cursor:pointer;":"color:#ccc;")+"font-family:Verdana>下一页&nbsp;&gt;&gt;</span>";
	...
	with(varMenuObj){
		...
		innerHTML="<div><h4 style='font-family:vernada;display:none;'>输入中文/拼音或→←↑↓选择</h4>"+(varHtmlStr==""?"<nobr>‘"+varObjValue+"’没有相应城市信息。</nobr>":varHtmlStr+varNextPageStr)+"</div>";
	}
...
//	var varThisObj=varMenuObj.obj;
	var varMenuValue=$(a)[0].title;
//	var hidObj = $F('hotelcity');
//	$(varThisObj).attr('city',varMenuValue[2]);
	console.debug(a);
	if($.cookie('city_type')=="0"){
		$.cookie('begin_city',$(a).attr('city'),{expires:9999})
	}else if($.cookie('city_type')=="1"){
		$.cookie('end_city',$(a).attr('city'),{expires:9999})
	}
